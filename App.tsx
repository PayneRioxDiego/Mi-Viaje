...
import { Search, MapPin, AlertTriangle, Loader2, ChevronRight, Camera, Grid, Map as MapIcon, Layers } from 'lucide-react';
...
interface TravelAnalysis {
  ...
  mapsLink?: string;
  openNow?: string;
  // --- AGREGADOS PARA EL MAPA ---
  lat?: number;
  lng?: number;
}

function App() {
  const [activeTab, setActiveTab] = useState<'analyze' | 'cards' | 'map'>('analyze');
  ...
  const fetchHistory = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // --- LIMPIEZA DE DATOS CRÍTICA PARA EL MAPA ---
        const safeData = data.map((item: any) => ({
            ...item,
            id: item.id || Math.random().toString(36).substr(2, 9),
            // Aseguramos que lat/lng sean números reales para el mapa
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      // Recargamos el historial para que el mapa se actualice al instante
      fetch('/api/history', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    return (
      <div className="group bg-white rounded-[2rem] overflow-hidden shadow-lg shadow-slate-200/50 hover:shadow-2xl transition-all duration-300 border border-slate-100 flex flex-col h-full">
        <div className="relative h-56 overflow-hidden shrink-0">
          {item.photoUrl ? (
            ...
            <div className="w-full h-full bg-slate-100 flex flex-col items-center justify-center text-slate-300">
              ...
              <span className="font-medium text-[10px]">Sin foto</span>
            </div>
          )}
          ...
          <div className="absolute top-4 left-4 right-4 flex justify-between items-start">
            <span className="bg-white/90 backdrop-blur-md text-slate-800 text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-wider shadow-sm">
          ...
          <div className={`absolute bottom-3 right-3 h-10 w-10 rounded-full flex flex-col items-center justify-center shadow-lg border-2 border-white ${getScoreBadge(item.score || 0)}`}>
            <span className="text-sm font-black leading-none">{item.score || 0}</span>
          </div>
        </div>
        <div className="p-5 flex flex-col flex-grow">
          ...
          <div className="bg-slate-50 p-3 rounded-xl mb-4 flex-grow">
            <p className="text-slate-600 text-xs leading-relaxed line-clamp-4">{mainSummary}</p>
          </div>
          <div className="flex items-center justify-between pt-3 border-t border-slate-100 mt-auto">
            ...
            {item.mapsLink ? (
                <a href={item.mapsLink} target="_blank" rel="noreferrer" className="flex items-center gap-1 px-3 py-1.5 bg-slate-900 text-white rounded-lg text-[10px] font-bold hover:bg-black transition-all shadow-md">
        ...
        <div className="max-w-6xl mx-auto px-4">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-slate-800 tracking-tight hidden sm:inline">Travel Hunter</span>
            </div>
            <nav className="flex bg-slate-100 p-1 rounded-xl">
              <button onClick={() => setActiveTab('analyze')} className={`px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-all ${activeTab === 'analyze' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                <Search className="h-4 w-4" /> <span className="hidden sm:inline">Analizar</span>
              </button>
              <button onClick={() => setActiveTab('cards')} className={`px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-all ${activeTab === 'cards' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                <Grid className="h-4 w-4" /> <span className="hidden sm:inline">Mis Hallazgos</span>
              </button>
              <button onClick={() => setActiveTab('map')} className={`px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-all ${activeTab === 'map' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                <MapIcon className="h-4 w-4" /> <span className="hidden sm:inline">Mapa Mundi</span>
              </button>
            </nav>
          </div>
        </div>
        ...
        {activeTab === 'analyze' && (
          <div className="animate-fade-in">
            <div className="text-center max-w-2xl mx-auto mb-10 mt-8">
              <h1 className="text-4xl font-black text-slate-800 tracking-tight mb-3">¿Joya o Trampa?</h1>
              <p className="text-slate-500">Pega un link de TikTok o Instagram para descubrirlo.</p>
            </div>
            <div className="max-w-2xl mx-auto mb-12">
              <div className="bg-white p-2 rounded-2xl shadow-xl shadow-indigo-100/50 flex flex-col sm:flex-row items-center border border-slate-100">
                ...
                <button onClick={handleAnalyze} disabled={loading || !url} className="w-full sm:w-auto mt-2 sm:mt-0 bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-xl font-bold transition-all flex items-center justify-center gap-2 shadow-lg shadow-indigo-200">
        ...
        {activeTab === 'cards' && (
          <div className="animate-fade-in">
             <div className="flex items-center justify-between mb-8">
                <h2 className="text-2xl font-black text-slate-800">Tu Colección de Viajes</h2>
                <span className="bg-indigo-100 text-indigo-700 font-bold px-3 py-1 rounded-full text-xs">{history.length} Destinos</span>
             </div>
             {history.length > 0 ? (
               ...
               <div className="text-center py-20 text-slate-400"><p>Aún no has guardado ningún viaje.</p></div>
             )}
          </div>
        )}
